...
const coursera = {
  ...
  //*[@id="courses"]/div/div/div/div[3]/div/div[1]/div[2]/div/div[2]/div[2]/a/span
  duration:
...
const scrapeCourseData = async (url) => {
  ...
  try {
    ...
    if (
      await checkElementExistence(page, "xpath///div[2]/div/button/span/span")
    ) {
    ...
    if (
      await checkElementExistence(page, "xpath///div[3]/div/div[1]/button/span")
    ) {
      await page.click("xpath///div[3]/div/div[1]/button/span");
      animateur = await extractAnimateur(page, coursera.animateur);
    } else {
      animateur =
        (await extractAnimateur(page, coursera.animateur).length) > 0
          ? await extractAnimateur(page, coursera.animateur)
          : await extractText(
              page,
              '//div/div[2]/div/a[@data-track-component="hero_instructor"]/span'
            );
    }
